#!/usr/bin/env bash
# =============================================================================
# SKIP_V2 EXPERIMENT — finetune under skip_phase v2 schedule (ah64/cpp2)
# NOT the OFFSET experiment. Offset entry: train_finetune_offset.sh
# Output: .../aha-wam-runs/train/skip_v2_ft_*
# Eval after train: train_skip_phase.sh (VIDEO_DIT_MODE=skip_phase) vs old baseline
# =============================================================================
# 平台：算法包 cwr_wulan_algorithm，入口选本脚本。1 机 x 8 卡，超时 10–12h。
set -euo pipefail

REVISION="2026-09-08-skip-v2-ft-10h-fastcache"
echo "train_finetune_skip_v2.sh revision: ${REVISION}"
echo "EXPERIMENT=SKIP_V2 (train only; no robotwin eval in this job)"

# ===================== 全部写死（平台不用 set）=====================
SMOKE=0
SMOKE_MAX_STEPS=30
# Conservative ~10h budget: prefer finishing under 10h if steps are slow.
# Assume ~7–9 s/optimizer step on 8×A800 → 4000 steps ≈ 8–10h train (+ startup).
FULL_MAX_STEPS=4000
SMOKE_SAVE_EVERY=10
FULL_SAVE_EVERY=500
LOG_EVERY=10
...
MACHINE_RANK=0
MASTER_PORT=29501
# 11h wall: enough for 4000 steps + load; not a 12h stretch goal
JOB_TIMEOUT_S=39600

TASK_NAME="robotwin_ahawam_skip_v2"
# Standard ah=64 / num_frames=65 (lighter than OFFSET's 97). Still use conservative bs.
BATCH_SIZE="4"
GRAD_ACCUM="4"
NUM_WORKERS="8"

HARDCODED_OUTPUT_DIR="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/train/skip_v2_ft_smoke"
HARDCODED_OUTPUT_DIR_FULL="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/train/skip_v2_ft_8x10h"
# ==================================================================
...
JOB_LOG="${OUTPUT_DIR}/train_finetune_skip_v2_${RUN_TAG}_${RUN_TS}.log"
...
echo "TRAIN_BUDGET_HINT=~10h conservative (max_steps=${MAX_STEPS}, timeout=${JOB_TIMEOUT_S}s)"
...
echo "6. NOT offset task (task=${TASK_NAME}, num_frames=65 path)"
echo "7. THIS JOB = TRAIN ONLY (eval is separate: train_skip_phase.sh)"
"${PYTHON}" - <<PY
