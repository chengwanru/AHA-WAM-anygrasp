#!/usr/bin/env bash
# =============================================================================
# EVAL after SKIP_SUCCESS_BC — skip_phase ONLY (ah64/cpp2), success-BC ckpt
# Baseline / released zero-shot skip / skip_v2_ft = reuse existing tables (do NOT re-run).
# Task set = MAIN-2 tasks (19 × 40 ep; put_bottles_dustbin excluded).
# Checkpoint: skip_success_bc_8x4k/checkpoints/weights/step_004000.pt
# =============================================================================
# 提交：算法包 cwr_wulan_algorithm，入口选 train_eval_skip_success_bc.sh
# batch2：入口选 train_eval_skip_success_bc_batch2.sh（另一台 8 卡，另外 10 个 MAIN-2 任务）
# 挂载：wulann + wulann2 + wulann3 + wulann4；lavapipe 硬扛时超时建议 ≥48h（JOB_TIMEOUT_S=172800）
set -euo pipefail

echo "train_eval_skip_success_bc.sh revision: 2026-09-10-success-bc-lavapipe-v12-epidx"
# ===================== 写死配置（你不用 set）=====================
...
# Finetuned ckpt (cluster path; explore fallback applied later)
export SKIP_PHASE_CKPT="${SKIP_PHASE_CKPT:-/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/train/skip_success_bc_8x4k/checkpoints/weights/step_004000.pt}"
export SKIP_PHASE_DATASET_STATS="${SKIP_PHASE_DATASET_STATS:-/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/train/skip_success_bc_8x4k/dataset_stats.json}"
# Only skip_phase: compare vs MAIN-2 baseline / released+FSM / skip_v2_ft+FSM.
EVAL_MODES=(skip_phase)

case "${SKIP_PHASE_BATCH}" in
batch1)
    # 9 tasks with MAIN-2 baseline (exclude put_bottles_dustbin — not in D.2 table)
    HARDCODED_OUTPUT_DIR="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/robotwin/video_dit_skip_phase_smoke_v12_lvp_success_bc"
    HARDCODED_OUTPUT_DIR_FULL="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/robotwin/video_dit_skip_phase_40eps_v12_lvp_success_bc"
    SKIP_PHASE_TASKS=(
    ...
    ;;
batch2)
    echo "train_eval_skip_success_bc.sh revision: 2026-09-10-success-bc-lavapipe-v12-epidx-batch2"
    HARDCODED_OUTPUT_DIR="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/robotwin/video_dit_skip_phase_smoke_v12_lvp_success_bc_batch2"
    HARDCODED_OUTPUT_DIR_FULL="/opt/huawei/dataset/cwr_dataset_wulann4/aha-wam-runs/robotwin/video_dit_skip_phase_40eps_v12_lvp_success_bc_batch2"
    SKIP_PHASE_TASKS=(
...
# lavapipe 全量 19×40 很慢；默认 48h，平台超时请 ≥ JOB_TIMEOUT_S
export JOB_TIMEOUT_S="${JOB_TIMEOUT_S:-172800}"  # 48h — lavapipe CPU render
...
print(f"EVAL CKPT (skip_success_bc): {ckpt}")
...
if batch == "batch2":
    ...
    # Must match SKIP_PHASE_TASKS batch1 (MAIN-2 baseline rows; no put_bottles_dustbin)
    expected = [
